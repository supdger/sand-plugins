...
final class FailureAuditWriter
{
    ...
    public function write(?string $documentId, string $context, string $idempotencyKey, string $requestId, string $code, int $status): void
    {
        try {
            ...
            $stmt->execute([':document'=>$documentId,':action'=>$this->config->action,':outcome'=>'failed',':request'=>$requestId,':context_hash'=>$context===''?null:hash('sha256',$context),':key_hash'=>$idempotencyKey===''?null:hash('sha256',$idempotencyKey),':code'=>$code,':status'=>$status]);
        } catch (\Throwable) {
