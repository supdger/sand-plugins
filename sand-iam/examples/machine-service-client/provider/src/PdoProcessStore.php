...
final class PdoProcessStore implements ProcessStore
{
    ...
    public function process(array $claims, string $documentId, string $key, string $requestId): array
    {
        ...
        try {
            ...
            $action = $this->action($claims);
            if ($existing !== null) return $this->finishReplay($existing, $org, $client, $documentId, $key, $requestId, (string) $claims['context_id'], $action, $hash, $bytes);
            ...
            if (!is_array($created)) {
                ...
                return $this->finishReplay($existing, $org, $client, $documentId, $key, $requestId, (string) $claims['context_id'], $action, $hash, $bytes);
            }
            $result = ['process_id'=>(int)$created['id'], 'document_id'=>$documentId, 'content_sha256'=>$hash, 'content_bytes'=>$bytes, 'replayed'=>false];
            $this->audit($org, $client, $documentId, $key, $requestId, (string)$claims['context_id'], $action, false);
    ...
    private function finishReplay(array $old, int $org, int $client, string $document, string $key, string $request, string $context, string $action, string $hash, int $bytes): array
    {
        ...
        $this->audit($org, $client, $document, $key, $request, $context, $action, true); $this->db->commit();
        return ['process_id'=>(int)$old['id'], 'document_id'=>$document, 'content_sha256'=>$hash, 'content_bytes'=>$bytes, 'replayed'=>true];
    }
    private function audit(int $org, int $client, string $doc, string $key, string $request, string $context, string $action, bool $replayed): void
    {
        ...
        foreach ([':org'=>$org, ':client'=>$client, ':doc'=>$doc, ':action'=>$action, ':outcome'=>'succeeded', ':request'=>$request, ':context'=>$context, ':key_hash'=>hash('sha256',$key)] as $name=>$value) $stmt->bindValue($name, $value);
        $stmt->bindValue(':replayed', $replayed, \PDO::PARAM_BOOL);
        $stmt->execute();
    }
    /** @param array<string,mixed> $claims */
    private function action(array $claims): string
    {
        $actions = $claims['actions'] ?? null;
        if (!is_array($actions) || count($actions) !== 1 || !is_string($actions[0] ?? null)) {
            throw new ProviderException('PROVIDER_B_CONTEXT_CLAIMS_INVALID', 403);
        }
        ProviderProtocol::action($actions[0]);
        return $actions[0];
    }
