...
return [
    ...
    'chains' => [
        ...
        'human-auth-session-mfa' => [
            ...
            'live_credential_slots' => ['platform_admin'],
            'live_prerequisites' => ['平台管理员可创建本轮专用客户主体、应用和认证策略', '运行时认证 pepper 与 MFA 加密密钥已临时配置', '应用用户凭证只由本轮注册、登录和 MFA 验证响应捕获'],
            'cleanup' => '撤销本轮三条会话及 MFA 因子，按 challenge、恢复码、因子、刷新令牌、会话、认证凭据、身份和认证策略顺序删除；最后停用本轮应用与客户主体审计锚点并核对 active residual 为零。',
        ],
        ...
        'event-webhook-delivery' => [
            ...
            'live_credential_slots' => ['platform_admin'],
            'live_prerequisites' => ['启用 audit event outbox 与投递 worker', '受控公共 HTTPS 接收器', '既有应用环境与调用身份'],
            'cleanup' => '撤销并删除本轮调用凭证和 Webhook，删除投递与接收器临时配置，并核对双侧零残留。',
        ],
    ],
];
