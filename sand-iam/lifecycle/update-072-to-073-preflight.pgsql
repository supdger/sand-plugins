...
BEGIN
    IF to_regclass(current_schema() || '.sand_iam_schema_migration') IS NULL THEN
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires the completed 0.7.2 migration ledger through 040';
    END IF;
    ...
    IF (SELECT count(*) FROM information_schema.columns
       ...
       OR NOT EXISTS (
        ...
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires the exact migration ledger structure';
    END IF;
    ...
    IF ledger_pk IS DISTINCT FROM 'PRIMARY KEY (migration_file)' THEN
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires the exact migration ledger primary key';
    END IF;
    ...
    IF matched_columns <> 1 THEN
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires the completed nullable service-grant data-class column';
    END IF;
    ...
    IF matched_constraints <> 1 THEN
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires the service-grant data-class constraint';
    END IF;
    ...
    IF matched_indexes <> 1 THEN
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires the completed revision 038 retention index';
    END IF;
    ...
    IF matched_constraints <> 1 THEN
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires the completed revision 040 passkey constraint';
    END IF;

    CREATE TEMPORARY TABLE sand_iam_073_expected_ledger (
    ...
    INSERT INTO sand_iam_073_expected_ledger (migration_file, revision, checksum, package_version) VALUES
    ...
    SELECT count(*) INTO expected_rows FROM sand_iam_073_expected_ledger;
    SELECT count(*) INTO recorded_rows FROM sand_iam_schema_migration;
    IF expected_rows <> 41
       OR recorded_rows <> expected_rows
       OR (SELECT max(revision) FROM sand_iam_schema_migration) <> 40
       OR EXISTS (
            SELECT 1 FROM sand_iam_schema_migration recorded
            FULL OUTER JOIN sand_iam_073_expected_ledger expected USING (migration_file)
        ...
        RAISE EXCEPTION 'SandIAM 0.7.3 update requires exact 001-040 ledger identities before executing 041';
    END IF;
END $$;
