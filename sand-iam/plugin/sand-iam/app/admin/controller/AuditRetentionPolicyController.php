    ...
    protected function normalizePayload(array $payload, ?object $existing = null): array
    {
        foreach (['archive_after_days', 'retention_days', 'alert_window_seconds', 'alert_failure_threshold'] as $field) {
            ...
            if (!is_int($value) && !(is_string($value) && preg_match('/^[0-9]+$/D', $value) === 1)) {
                $code = str_starts_with($field, 'alert_') ? 'SAND_IAM_SECURITY_ALERT_POLICY_INVALID' : 'SAND_IAM_AUDIT_RETENTION_INVALID';
                throw new ApiException($code . ': 策略天数、窗口和次数必须是整数', 400);
            }
            $payload[$field] = (int) $value;
        }
        if (array_key_exists('purge_enabled', $payload)) {
            if (!in_array($payload['purge_enabled'], [0, 1, '0', '1', false, true], true)) {
                throw new ApiException('SAND_IAM_AUDIT_RETENTION_INVALID: 清除开关必须明确为开启或关闭', 400);
            }
            $payload['purge_enabled'] = (int) $payload['purge_enabled'];
        }
        return $payload;
    }
