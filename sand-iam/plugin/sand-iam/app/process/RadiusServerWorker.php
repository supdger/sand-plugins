...
final class RadiusServerWorker
{
    /** @var resource|null */
    private $socket = null;
    ...
    public function onWorkerStart(): void
    {
        ...
        $port = (int) config('plugin.sand-iam.app.radius_auth_port', 1812);
        if (filter_var($host, FILTER_VALIDATE_IP) === false || $port < 1 || $port > 65535) throw new \RuntimeException('SandIAM RADIUS bind configuration invalid');
        ...
        if (!is_resource($socket)) throw new \RuntimeException('SandIAM RADIUS socket unavailable: ' . $errorCode);
        ...
        Timer::add(0.01, function (): void {
            ...
            for ($i = 0; $i < 50; $i++) {
                ...
                try {
                    $ip = $this->peerIp($peer);
                    $response = (new RadiusAccessService())->handle($ip, $packet);
                    if ($response !== null) stream_socket_sendto($this->socket, $response, 0, $peer);
                } catch (\Throwable $exception) {
                    Log::error('SandIAM RADIUS packet handling failed', ['exception_type' => $exception::class, 'exception_file' => basename($exception->getFile()), 'exception_line' => $exception->getLine()]);
                }
            }
        });
    }
    ...
    private function peerIp(string $peer): string
    {
        if (str_starts_with($peer, '[')) {
            $end = strpos($peer, ']');
            return $end === false ? '' : substr($peer, 1, $end - 1);
        }
