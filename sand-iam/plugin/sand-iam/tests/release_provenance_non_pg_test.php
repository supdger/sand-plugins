...
$passed = sandIamWithIsolatedFixture($root, static function (string $fixtureRoot) use ($run, $git): bool {
    ...
    if (file_put_contents($attributes, $controllerRelative . " export-ignore\n") === false
    ...
    if (!copy($fixtureRoot . '/' . $controllerRelative, $stageRoot . '/sand-iam/' . $controllerRelative)) return false;
    ...
    return $vendor && $dist && $lock && $contract && $injection && $stageParity && $formalBuild && $normalBuilderZip && $unsupportedTreeInputs;
});
