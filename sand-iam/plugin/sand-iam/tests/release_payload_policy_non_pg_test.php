...
$governanceGate = $currentHygieneStatus !== 0
    && str_contains($currentHygieneOutput, 'project license exists')
    && str_contains($currentHygieneOutput, 'concrete private security reporting channel exists')
    && str_contains($currentHygieneOutput, 'exactly one contribution governance mechanism is selected')
    && sandIamWithIsolatedFixture($root, static function (string $fixtureRoot) use ($runHygiene): bool {
        if (file_put_contents($fixtureRoot . '/LICENSE', "Approved project license fixture\n") === false) return false;
        if (file_put_contents($fixtureRoot . '/SECURITY.md', "\nPrivate reports: https://github.com/example/sand-iam/security/advisories/new\n", FILE_APPEND) === false) return false;
        if (file_put_contents($fixtureRoot . '/CONTRIBUTING.md', "\nDeveloper Certificate of Origin\nSigned-off-by: Contributor <contributor@example.test>\n", FILE_APPEND) === false) return false;
        [$status, $output] = $runHygiene($fixtureRoot);
        return $status === 0 && str_contains($output, 'Release payload hygiene: passed=14/14; failed=0');
    });
...
$checks = [
    ...
    'license, private security channel, and one contribution mechanism fail closed' => $governanceGate,
];
