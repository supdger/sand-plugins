...
> 当前交付顺序按[完整开源成品交付计划](../development/sand-iam-terminal-release-runbook.md)执行；本文定义能力和最终条件，不要求先完成所有宿主与发布条件再开发业务。

> 状态：2026-08-21 冻结为终极产品方向。本文描述最终产品，不代表当前实现已经完成。48 个验收原子的稳定 ID、计数和证据边界见[终极验收原子账本](../development/sand-iam-terminal-acceptance-ledger.md)；截至 2026-09-16，当前为 `29/48（60.4%）`，其中 D03 由当前 demo 的正式生命周期与激活证据通过，不代表七条业务链或四角色页面体验已经通过。
...
“比直接使用 Casdoor 更顺手”不是设计自评。三条开发者旅程必须按[开发者旅程验收](../development/sand-iam-developer-journey-acceptance.md)留下双方真实结果、安全差异、失败和清理证据。只有同一名独立开发者在同等环境完整计时并记录操作数，才允许附加定量效率结论。
...
├── 客户主体管理员：受委派主体内的应用、环境、身份与授权治理
└── 应用管理员：获授应用的身份源、用户、角色、策略、客户端和审计

SandIAM 运行平面
├── 独立应用用户：注册、登录、恢复、MFA、会话、自助资料
...
| 用户体验 | 平台管理员、客户主体管理员、应用管理员、独立应用用户四类清晰入口；只展示有用字段和可恢复错误 |
...
4. 至少一个受控真实机器调用服务和一个非 AI 业务应用真实接入；机器服务可采用非 AI 服务，SandIAM 不等待 SandAI；
5. 安装、升级、卸载、备份恢复、密钥轮换、并发、限流和安全基线通过；
6. 源码实现、正式 FLOW、本地业务闭环、可部署、已部署和线上验证分别留证。
7. 三条开发者旅程在 SandIAM 和 Casdoor 的同等环境留下结果、安全差异、失败和清理证据；SandIAM 按绝对目标验收，Casdoor 的不足作为对照结果而不反向阻塞 SandIAM。只有双方计时和操作数完整且满足定量门槛时，才可宣称“更顺手”。
