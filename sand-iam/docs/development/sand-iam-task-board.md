...
- **完整目标已闭合**：当前权威状态为 **R 9/9、P 20/20、F 7/7、
  L 4/4、D 8/8，FLOW 48/48**；七条业务链 **7/7**，十项发布门槛
  **10/10**。本目标不包含公开 push、正式 Release、生产部署或线上验证；
  这些动作均未执行。
- **R09 已通过**：用户确认每个编号业务步骤是核心人工操作单位，必要的连续
  表单提交和验证是该步骤的可追溯子动作，额外重试、恢复、命令和代码修改
  另计；规范与 `operation-unit-decision.json` 已版本化。v36 的 SandIAM
  三条旅程各两轮均通过，Casdoor 保持相对能力与风险观察，不声明定量优越性。
  validator 复跑为 3 条旅程、12 轮、48 份证据全部通过。
- **J2 与 J3 补证已关闭**：J2 两轮错误 TOTP 均是步骤 4 冻结要求内的负向
  子动作，错误 challenge 则是原始账本已标识的自动负向探针；两轮请求和事件
  摘要未改。J3 发现宿主 `CheckAuth` 的 `SystemException` 被 SandIAM
  Handler 错映射为 500，权威源码已将该函数权限拒绝稳定映射为 HTTP/code
  403，并保留未知异常的脱敏 500。两轮临时受限角色均未获得
  `sand_iam:grant:save`，保存请求均 403，临时用户、角色和令牌清理为零残留。
  Astra/high 有界复核 **ACCEPT（P0/P1/P2=0）**。
- **稳定性已通过，计时不重启**：连续稳定性固定从
  `2026-09-18 00:44:34 +08:00` 起算，至 `09:29:40` 连续
  `31506.007` 秒（8 小时 45 分 06 秒），526 个样本、**3682/3682**
  检查通过、0 失败。v36→定向修复候选的 704 个包文件中只有
  `plugin/sand-iam/app/exception/Handler.php` 改变；修复只改变已拒绝管理
  请求的响应映射，不改变 worker 生命周期、持久化、队列、授权状态、协议
  路径或资源采样。两轮受影响路径定向复验通过，独立复核认可影响承接，
