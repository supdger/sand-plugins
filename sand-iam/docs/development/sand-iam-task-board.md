...
- **2026-09-18 R09 与稳定性口径修正**：Casdoor 是相对能力和风险对照，
  SandIAM 按自身绝对结果、安全、时间、操作数和清理目标验收；Casdoor 未达到
  SandIAM 安全目标时登记为对照差距，不再反向判定 SandIAM 失败。只有双方
  计量完整时才允许附加定量效率结论。校验模板和验证器升级为
  `sand-iam.casdoor-comparison/v3`。连续稳定性门槛按用户决定由 24 小时改为
  同一最终候选 8 小时（`28800` 秒）。v35 已有 F07、七链和 8 小时结果，但
  R09 的 SandIAM 人工操作/命令计量不完整，J1 第一轮还混入一次早期失败尝试；
  因此当前权威状态为 **47/48、7/7、9/10**，只剩 R09 计量补跑。上述源码
  修正会形成新候选，候选绑定门槛须按影响范围重新核对，不能沿用错误的
  “48/48、10/10”状态文件。
