...
- 用户已确认并版本化操作数口径：一个编号业务步骤是一个核心人工操作，达成
  该步骤所需的连续提交是可追溯子动作；编号步骤之外的命令、代码修改、非预
  声明重试和恢复另计，自动负向探针单列。J2 两轮错误 TOTP 都位于步骤 4
  的冻结必需证据内；错误 challenge 已在原始账本标为自动负向探针。派生裁决
  绑定两轮原始请求和事件摘要，未改写事件、请求或时间戳。
- J3 原有证据已覆盖跨应用拒绝、应用内具备权限成功和委派撤销后拒绝；补充
  两轮真实受限角色证明 `sand_iam:grant:save` 未分配时授权保存稳定返回
  HTTP/code 403，用户、角色和令牌均自动清理且零残留。为此修复的候选相对
  v36 的 704 个包文件只改变 `Handler.php`，安装摘要与实际 ZIP 一致。
- Casdoor 仅作相对能力与风险观察，其安全差距不反向判定 SandIAM 失败；
  当前不声明双方定量效率优越性。正式 validator 复跑通过 3 条旅程、12 轮、
  48 份直接引用证据。Astra/medium 与最终 Astra/high 均给出
  **ACCEPT**；最终有界复核为 **P0/P1/P2=0/0/0**。
- 当前严格计分为 **R 9/9、P 20/20、F 7/7、L 4/4、D 8/8，
  48/48**；七条业务链 **7/7**，发布门槛 **10/10**。
- 连续稳定性门槛按用户后续决定调整为同一最终候选连续 **8 小时
  （28800 秒）**；此前 24 小时口径不再作为当前完成条件。计时固定从
  `2026-09-18 00:44:34 +08:00` 开始，到 `09:29:40` 共
  `31506.007` 秒（8 小时 45 分 06 秒），526 个样本、**3682/3682**
  检查通过、0 失败。v35→v36 的 704 个包文件中 703 个字节一致，唯一差异为
  `CHANGELOG.md` 的时长文字，运行载荷未变。后续函数权限异常修复只改变
  `Handler.php` 的拒绝响应映射，受影响路径已两轮复验，未改变 worker、
  持久化、队列、授权状态、协议或资源采样；独立复核接受影响承接并明确
  `restart_required=false`。后来误启动的重复轮次不用于验收，不能覆盖或
  重置原起算时间。
