...
`0.7.2` 保持既有 `001–038` 均不可变，并新增自校验迁移 `039`：根与包内 `021_admin_permission_catalog.pgsql` 必须等于已发布 0.6.0 的 SHA-256；035 账本的已知文件名、修订号、哈希和包版本必须完整一致，036–039 的所有自引用校验值必须等于将该值规范化为 `__SELF_SHA256__` 后的 SHA-256。fresh install 完整带入 `001–039`；正常 `0.7.1 → 0.7.2` 先精确核验 001–038 的 39 条账本身份、账本结构、服务授权数据分级列和 038 保留索引，再只内联 039。成功时账本必须为 40 行且最大修订号为 39；未知、多余、缺失或冲突账本行均拒绝继续。SandPackage 的通用升级选择与已安装状态收养不在本插件范围，必须在宿主侧先独立修复和验收。
