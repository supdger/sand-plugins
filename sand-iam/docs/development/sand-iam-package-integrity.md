...
这里的“版本一致性”只指可安装插件包的候选发行版本（当前为根与插件 `info.ini`、运行配置、门户及随包 Composer 根包元数据中的 `0.7.3`）。管理 OpenAPI 目录的 `info.version=0.13.0-candidate` 是独立的接口契约版本，不与插件包版本比较，也不能据此推断已发布或已部署；它的值和口径由[管理端接口交接](sand-iam-management-api-v0.1.md)冻结。
...
`0.6.0 → 0.7.0` 的 `ledger_absent` 恢复场景。0.7.3 的正常载荷策略显式排除这两个文件，
也禁止构建工具依据当前 0.7.3 元数据重写它们。SandPackage 因而可以将 0.7.3 作为正常升级包
...
`0.7.3` 保持既有 `001–040` 均不可变，并新增自校验迁移 `041`：根与包内 `021_admin_permission_catalog.pgsql` 必须等于已发布 0.6.0 的 SHA-256；035 账本的已知文件名、修订号、哈希和包版本必须完整一致，036–041 的所有自引用校验值必须等于将该值规范化为 `__SELF_SHA256__` 后的 SHA-256。fresh install 完整带入 `001–041`；正常 `0.7.2 → 0.7.3` 先精确核验 001–040 的 41 条账本身份、账本结构及修订 039/040 的目标结构，再内联 041。成功时账本必须为 42 行且最大修订号为 41；未知、多余、缺失或冲突账本行均拒绝继续。SandPackage 的通用升级选择与已安装状态收养不在本插件范围，必须在宿主侧先独立修复和验收。
