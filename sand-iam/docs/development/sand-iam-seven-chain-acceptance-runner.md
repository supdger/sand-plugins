...
- 第 7 链订阅生产 `credential.changed`，由正常 `credential.issue` 触发。创建端点返回的一次性密钥只进入本轮受控接收器临时配置；接收器校验 HMAC 和真实凭证事件 envelope，第一次返回 500、人工重试后返回 204。完成后撤销凭证、停用端点，按 **delivery → credential → endpoint** 清理，并独立删除接收器配置和证明双侧零残留。接收器必须运行在 SandIAM SSRF 边界允许的公共 HTTPS 地址；仓库内模板默认关闭。
