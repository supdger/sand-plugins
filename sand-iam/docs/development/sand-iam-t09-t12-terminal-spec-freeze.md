...
| R08-T10-03 | T10 | 同步连接和运行记录；应用管理员会话 | `sync_connector_id`、`cursor`、`conflict_policy`；`SAND_IAM_SYNC_CONFIGURATION_INVALID` 时保留失败页并修正后重试 | `sandadmin-artd/src/views/plugin/sand-iam/sync-connector/index.vue` | `sync-connector/index|read|save|update|disable|configure|test|run|runs` | 真实 PostgreSQL/企业目录或其他受控目录驱动；调度器 | 已映射的同应用组允许同步；未映射、跨主体、游标失败和异常停用比例拒绝/暂停并审计 | 失败后从同页重试，不越过游标；停用连接并清理测试连接、运行和来源投影 | 源码候选、待真实目录/调度/HTTP/页面；T10 第 6、7 节 |
