...
3. 仅在隔离环境临时启用该能力，使用一个受控对象验证加密、解密、允许、拒绝、撤销和审计；验证结束后关闭临时开关并保留脱敏结果。
4. 隔离验证不能授权生产启用。生产须另有变更计划、回滚方法、密钥版本、受影响 worker 和监控/审计负责人，经相应运维授权后才按计划启用。
5. 生产计划获批后，先打开功能开关；需要 worker 的能力最后打开 worker 开关，并按宿主流程重载进程。
6. 观察错误率、积压、Worker restart、连接池等待、RSS/FD 和拒绝审计；异常时先关闭 worker/功能开关，
   不删除队列、密文、审计或迁移账本。
