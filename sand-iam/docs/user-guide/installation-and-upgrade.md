...
此正常升级只适用于账本已精确完成 `001–040` 的 0.7.2 安装。SandPackage 会先核验 41 条账本
身份、账本结构、修订 039 的可空服务授权数据分级列和修订 040 的 Passkey challenge 约束，
再执行迁移 `041`。041 在固定锁顺序下检查身份授权、策略目标、策略版本、回滚来源和已发布
指针的存量归属；任何冲突都会关闭失败并回滚，不会自动删除、换绑或改写业务数据。

成功后策略动作列为 `varchar(96)`，身份关系和策略目标受同应用组合外键保护，策略版本及其
发布/回滚指针受同策略、同应用组合外键保护；迁移账本为 42 行且最大修订号为 41。
生成的 `update.sql` 将 0.7.3 preflight 与 041 放在同一个显式 PostgreSQL 事务中。
...
0.7.3 正常包没有旧 `0.6.0 → 0.7.0` recovery descriptor，不能借用历史描述器或手工重放 `update.sql`。
