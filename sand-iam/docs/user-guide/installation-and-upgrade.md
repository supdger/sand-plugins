...
- 数据库阶段已提交：先核对迁移账本是否已有 revision 38，再只按 SandPackage 显示的“数据库已提交后”恢复/完成路径处理。不要删除候选、伪造 FAILED 状态、改账本或用旧包覆盖文件。
- 无法判定阶段、没有可用的 SandPackage 恢复操作，或恢复操作报告失败：停止操作，保持备份和现场，向宿主/SandPackage 维护者提交版本、候选摘要、阶段、错误码、请求 ID 与脱敏日志。

0.7.1 正常包没有旧 `0.6.0 → 0.7.0` recovery descriptor，不能借用历史描述器或手工重放 `update.sql`。
