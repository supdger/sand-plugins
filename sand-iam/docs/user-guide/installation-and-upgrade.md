...
再执行迁移 `039`。039 只解除该列的非空约束，保留 `varchar(32)`、`internal` 默认值、既有数据
和检查约束；成功后账本必须有 40 行且最大修订号为 39。

生成的 `update.sql` 将 0.7.2 preflight 与 039 组合在同一个显式 PostgreSQL 事务中。缺失、多余、
摘要冲突或结构不匹配均在 039 前关闭失败；任一语句失败时整段事务回滚。不要从 0.7.0 跨版本直接
升级到 0.7.2，也不要手工清空 `data_class`、解除约束、补写 039 账本或重复执行 `update.sql`。
